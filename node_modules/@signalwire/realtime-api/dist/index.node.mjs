...
var voiceCallReceiveWorker = function* (options) {
  getLogger10().trace("voiceCallReceiveWorker started");
  const {
    channels: { swEventChannel },
    instanceMap,
    initialState
  ...
  getLogger10().trace("voiceCallReceiveWorker ended");
};
...
var voiceCallPlayWorker = function* (options) {
  getLogger11().trace("voiceCallPlayWorker started");
  const {
    channels: { swEventChannel },
    instanceMap: { get, set, remove },
    initialState
  ...
  function* worker(action) {
    ...
    switch (payload.state) {
      ...
      default:
        getLogger11().warn(`Unknown playback state: "${payload.state}"`);
        return false;
    }
  }
  while (true) {
  ...
  getLogger11().trace("voiceCallPlayWorker ended");
};
...
var voiceCallRecordWorker = function* (options) {
  getLogger12().trace("voiceCallRecordWorker started");
  const {
    channels: { swEventChannel },
    instanceMap: { get, set, remove },
    initialState
  ...
  function* worker(action) {
    ...
    switch (payload.state) {
      ...
      default:
        getLogger12().warn(`Unknown recording state: "${payload.state}"`);
        return false;
    }
  }
  while (true) {
  ...
  getLogger12().trace("voiceCallRecordWorker ended");
};
...
var voiceCallCollectWorker = function* (options) {
  getLogger13().trace("voiceCallCollectWorker started");
  const {
    channels: { swEventChannel },
    instanceMap: { get, set, remove },
    initialState
  ...
  while (true) {
    const action = yield sagaEffects8.take(
      swEventChannel,
  ...
  getLogger13().trace("voiceCallCollectWorker ended");
};
...
var voiceCallTapWorker = function* (options) {
  getLogger14().trace("voiceCallTapWorker started");
  const {
    channels: { swEventChannel },
    instanceMap: { get, set, remove },
    initialState
  ...
  function* worker(action) {
    ...
    switch (payload.state) {
      ...
      case "finished":
        ...
        return true;
      default:
        getLogger14().warn(`Unknown tap state: "${payload.state}"`);
        return false;
    }
  }
  while (true) {
  ...
  getLogger14().trace("voiceCallTapWorker ended");
};
...
var voiceCallConnectWorker = function* (options) {
  getLogger15().trace("voiceCallConnectWorker started");
  const {
    channels: { swEventChannel },
    instanceMap,
    initialState
  ...
  getLogger15().trace("voiceCallConnectWorker ended");
};
...
var voiceCallDialWorker = function* (options) {
  getLogger16().trace("voiceCallDialWorker started");
  const {
    instanceMap,
    channels: { swEventChannel },
    initialState
  ...
  getLogger16().trace("voiceCallDialWorker ended");
};
...
var voiceCallSendDigitsWorker = function* (options) {
  getLogger17().trace("voiceCallSendDigitsWorker started");
  const {
    channels: { swEventChannel },
    instanceMap: { get },
    initialState
  ...
  while (true) {
    const action = yield sagaEffects12.take(
      swEventChannel,
  ...
  getLogger17().trace("voiceCallSendDigitsWorker ended");
};
...
var voiceCallDetectWorker = function* (options) {
  getLogger18().trace("voiceCallDetectWorker started");
  const {
    channels: { swEventChannel },
    instanceMap: { get, set, remove },
    initialState
  ...
  function* worker(action) {
    ...
    switch (type) {
      ...
      case "fax":
        break;
      default:
        getLogger18().warn(`Unknown detect type: "${type}"`);
        break;
    }
    return false;
  }
  while (true) {
    const action = yield sagaEffects13.take(
      swEventChannel,
  ...
  getLogger18().trace("voiceCallDetectWorker ended");
};
...
var videoMemberWorker = function* (options) {
  getLogger19().trace("videoMemberWorker started");
  const {
    action: { type, payload },
    instanceMap: { get, set, remove }
  } = options;
  ...
  getLogger19().trace("videoMemberWorker ended");
};

// src/video/workers/videoRoomWorker.ts
var videoRoomWorker = function* (options) {
  var _b;
  getLogger20().trace("videoRoomWorker started");
  ...
  switch (type) {
    ...
    case "video.room.subscribed": {
      yield sagaEffects14.fork(MemberPosition.memberPositionWorker, __spreadProps(__spreadValues({}, memberPositionWorkerParams), {
  ...
  getLogger20().trace("videoRoomWorker ended");
};
...
var videoLayoutWorker = function* (options) {
  getLogger21().trace("videoLayoutWorker started");
  const {
    action: { type, payload },
    instanceMap: { get }
  } = options;
  ...
  getLogger21().trace("videoLayoutWorker ended");
};
...
var videoRoomAudienceWorker = function* (options) {
  getLogger22().trace("videoRoomAudienceWorker started");
  const {
    action: { type, payload },
    instanceMap: { get }
  } = options;
  ...
  getLogger22().trace("videoRoomAudienceWorker ended");
};
...
var videoRecordingWorker = function* (options) {
  getLogger23().trace("videoRecordingWorker started");
  const {
    action: { type, payload },
    instanceMap: { get, set, remove }
  } = options;
  ...
  getLogger23().trace("videoRecordingWorker ended");
};
...
var videoPlaybackWorker = function* (options) {
  getLogger24().trace("videoPlaybackWorker started");
  const {
    action: { type, payload },
    instanceMap: { get, set, remove }
  } = options;
  ...
  getLogger24().trace("videoPlaybackWorker ended");
};
...
var videoStreamWorker = function* (options) {
  getLogger25().trace("videoStreamWorker started");
  const {
    action: { type, payload },
    instanceMap: { get, set, remove }
  } = options;
  ...
  getLogger25().trace("videoStreamWorker ended");
};

// src/video/workers/videoCallingWorker.ts
var videoCallingWorker = function* (options) {
  getLogger26().trace("videoCallingWorker started");
  const {
    channels: { swEventChannel },
    initialState
  } = options;
  ...
  function* worker(action) {
    ...
    switch (type) {
      ...
      case "video.stream.ended":
        ...
        break;
      default:
        getLogger26().warn(`Unknown video event: "${type}"`);
        break;
    }
  }
  const workerCatchable = sagaHelpers.createCatchableSaga(worker, (error) => {
    getLogger26().error("Voice calling event error", error);
  });
  const isVideoEvent = (action) => action.type.startsWith("video.");
  while (true) {
    const action = yield sagaEffects15.take(
      swEventChannel,
      isVideoEvent
    );
    yield sagaEffects15.fork(workerCatchable, action);
  }
  getLogger26().trace("videoCallingWorker ended");
};
